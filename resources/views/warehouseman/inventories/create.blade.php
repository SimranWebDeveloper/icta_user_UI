    ...
    <div class="row mb-4">
        <div class="col-md-12 mb-4">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('warehouseman.inventories.store') }}" class="store-local-report-form">
                        @csrf
                                ...
                                <div class="col-md-4 form-group mb-3">
                                    ...
                                    <select frequency="true" id="products_select" name="products_id[]" class="form-control ui fluid search dropdown create_form_dropdown vendors_select_cl">
                                        ...
                                        @forelse($products as $item)
                                            <option value="{{$item->id}}" {{ old('products_id')==$item->id ? 'selected' : '' }}>{{ $item->categories->name }} - {{$item->product_name}}</option>
                                        @empty
                                            <option disabled selected>Məlumat yoxdur</option>
                                        @endforelse
                                    </select>
