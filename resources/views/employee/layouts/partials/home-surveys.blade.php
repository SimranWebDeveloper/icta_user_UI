        ...
        survey.surveys_questions.forEach((question, index) => {
            ...
            if (question.input_type === 'radio') {
                question.answers.forEach(option => {
                    optionsHtml += `
                      <div class="answers d-flex form-check">
                          <input class="form-check-input" type="radio" value="" name="question_${index}" id="option_${index}_${option.id}" />
                ...
                question.answers.forEach(option => {
                    optionsHtml += `
                      <div class="answers d-flex form-check">
                          <input class="form-check-input" type="checkbox" value="" name="question_${index}[]" id="option_${index}_${option.id}" />
                          <label class="w-100" for="option_${index}_${option.id}">${option.name}</label>
                      </div>
                  `;
                });
                ...
                optionsHtml = `
                  <div>
                      <textarea rows="7" cols="10" class="form-control" value="" name="question_${index}"></textarea>
                  </div>
              `;
            }
